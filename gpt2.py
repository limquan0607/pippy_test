...
def run(args):
    ...
    example_input_microbatch = torch.randint(0, gpt2.config.vocab_size, device = args.device,  dtype=torch.int64, requires_grad = False, size = (args.batch_size // args.chunks, 768))
    ...
    split_spec = {
        f'h.{i * decoders_per_rank}': SplitPoint.BEGINNING
  ...
  # 10 inoputs, 
    x = torch.randint(0, gpt2.config.vocab_size, device = args.device,  dtype=torch.int64, requires_grad = False, size = (args.batch_size, 768))
